...
## 2026-09-04: go module dependency CVEs need a Go toolchain to fix

PR #7's image scan (Grype) flagged fixed-version CVEs in four Go module
dependencies: `golang.org/x/net` (v0.55.0, fix in 0.56.0, GO-2026-5942),
`golang.org/x/text` (v0.37.0, fix in 0.39.0, GO-2026-5970),
`golang.org/x/crypto` (v0.52.0, fix in 0.55.0/0.56.0, GO-2026-6303 plus
three Unknown-severity advisories GO-2026-5932/6354/6355), and
`github.com/klauspost/compress` (v1.17.4, fix in 1.18.7, GO-2026-5841).
These can't be fixed by hand-editing `go.mod`'s version numbers: `go.sum`
holds cryptographic checksums that must be regenerated by running actual
Go tooling, which wasn't available in the environment this backlog entry
was written from. Run locally:

```bash
go get -u golang.org/x/net golang.org/x/text golang.org/x/crypto github.com/klauspost/compress
go mod tidy
